...
/**
 * DON'T USE THIS DIRECTLY, see aEditorCk
 * 
 * Copied from CKEditor's PHP class because the darn thing is full of private methods and
 * cannot be overridden in any constructive way. We needed to adjust it to not force
 * script tag wrappers on everything, breaking dynamic loading logic
 */
class aCkEditorOverrideable
{
	/**
  ...
  /**
   * Return just the textarea markup, in a fashion compatible with the editor method
   * of the parent class.
   */
	public function textareaMarkup($name, $value = "", $config = array(), $events = array())
	{
		$attr = "";
		foreach ($this->textareaAttributes as $key => $val) {
			$attr.= " " . $key . '="' . str_replace('"', '&quot;', $val) . '"';
	...
	/**
	 * Return just the ckeditor JS code that overrides the textarea, in a fashion compatible
	 * with the editor method of the parent class. There is no script tag wrapper.
	 */
	public function editorJavascript($name, $value = "", $config = array(), $events = array())
	{
  	$_config = $this->configSettings($config, $events);
